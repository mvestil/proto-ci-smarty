		...
		if ($this->_base_check()) {
			...
			if ($error_code == NO_ERROR) {
				...
				if ($result['count'] > 0) {
					...
					//print_r($_POST) ; die();
					$this->email->from('unlimitedincome@honestmailer.biz', 'Katherine Miller');
					$this->email->reply_to('titaquiapo@gmail.com');
					$this->email->to(null);
					$this->email->bcc($mmgemails); 
		...
		if ($this->_base_check()) {
			...
			if ($error_code == NO_ERROR) {
				...
				if ($result['count'] > 0) {
					...
					$this->email->from('unlimitedincome@honestmailer.biz', 'Risk-Taker Gain');
					//$this->email->reply_to('sarahlabandera@gmail.com');
					$this->email->bcc($mmgemails); 
