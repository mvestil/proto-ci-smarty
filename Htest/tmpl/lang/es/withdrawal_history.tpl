...
<tr>
 ...
 <td align=right>
Desde: <select name=month_from class=inpts>
...
Hasta: <select name=month_to class=inpts>
...
<tr>
 ...
 <td class=inheader>Typo</td>
 <td class=inheader>Credito</td>
 <td class=inheader>Debito</td>
...
<tr>
 ...
 <td align=right><b>
  ...
  {if $trans[trans].transtype eq 'Withdrawal request'} <a href=?a=cancelwithdraw&id={$trans[trans].id} onclick="return confirm('¿Estás seguro de que quieres eliminar esta petición?')">[cancel]</a>{/if}
  {else}
   
  {/if}
 </td>
...
{else}
<tr>
 <td colspan=6 align=center>No se han encontrado transacciones.</td>
</tr>
{/if}
<tr><td colspan=3>&nbsp;</td>
...
{if $qtrans > 0}
<tr>
 <td colspan=2>Para este período:</td>
...
<tr>
 <td class=inheader>Typo</td>
 <td class=inheader width=200>Cantidad</td>
 <td class=inheader width=170>Fecha</td>
</tr>
{if $qtrans > 0}
{section name=trans loop=$trans}
<tr>
 <td><b>{$trans[trans].transtype}</b></td>
 <td width=200 align=right><b>{$currency_sign} {$trans[trans].actual_amount}</b> <img src="images/{$trans[trans].ec}.gif" align=absmiddle hspace=1 height=17> {if $trans[trans].transtype eq 'Withdrawal request'} <a href=?a=cancelwithdraw&id={$trans[trans].id} onclick="return confirm('¿Estás seguro de que quieres eliminar esta petición?')">[cancel]</a>{/if}</td>
